//! Content-stream interpreter — **the core value of the PDF engine**.
//!
...
use crate::formats::pdf::ir::{BBox, Char, FontRef, ImageRef, Rect, Rule, Warning, WarningKind};
use crate::shared::base64::encode as base64_encode;
...
/// We NFKC just these blocks — deliberately not the whole string, so ①②,
/// full-width forms, and ligatures stay byte-exact.
...
/// by `ObjectId` so a form invoked many times is decoded and has its fonts/CMaps
/// built **once**, not per `Do`.
struct CachedForm {
    bytes: Vec<u8>,
